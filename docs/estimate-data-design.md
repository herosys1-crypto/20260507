...
직접 만든 견적서를 데이터에 넣는 방법:

1. 직접 만든 `.xlsx` 견적서를 `data/raw_estimates/견적서/업체명` 폴더에 복사한다.
2. 웹 화면 왼쪽의 `데이터 갱신` 버튼을 누른다.
3. 검색 결과에 새 견적서 품목이 반영됐는지 확인한다.

## 부가세 포함가와 별도가 처리

케이제이씨 견적서처럼 부가세 별도 단가와 부가세 포함 금액을 나누어 쓰는 양식은 아래 구조를 사용한다.

```text
G열: 부가세 별도 단가
H열: 부가세
I열: 부가세 포함 금액
J열: 낱개 부가세 포함 기준가 또는 확인용 가격
```

부가세 포함 기준 판매가를 단순히 `1.1`로 나누면 소수점 단가가 생긴다. 그래서 화면의 새 견적 초안에서는 `VAT incl`에 포함 기준 판매가를 넣으면 `VAT excl`을 1,000원 단위의 가까운 별도 단가로 자동 조정한다.
...
실제 견적서에는 조정된 별도 단가가 들어가고, 엑셀 수식으로 부가세와 포함 금액을 계산한다. 사용자는 필요하면 `VAT excl`을 직접 수정해서 마진이나 거래처 기준에 맞출 수 있다.

운영 원칙:

- 기준 판매가는 `VAT incl`에 입력한다.
- 견적서에 들어갈 실제 별도 단가는 `VAT excl`에서 확인한다.
- 소수점 별도 단가는 사용하지 않는다.
- 포함 기준가와 실제 포함 단가의 차이는 별도 단가 조정 결과로 관리한다.
- 새로 생성한 견적서는 자동으로 `data/raw_estimates/견적서/거래처명`에 보관되고, 다음 검색 데이터에 반영된다.
